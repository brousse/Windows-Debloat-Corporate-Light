#-------------------------------------------------------------------------------------------------------------------------
# Win10/Win11 Initial Setup Script
# Author: Brousse
# Origin: Simplified code from Disassembler <disassembler@dasm.cz> (n1snt/Windows-Decrapifier)
...
Write-Output "Uninstalling current user apps"
foreach ($app in $apps) {
    Write-Host "- Removing $app : " -NoNewLine
	if (Get-AppxPackage -Name $app) {
		Get-AppxPackage -Name $app | Remove-AppxPackage
		Write-Host "OK (installed)" -Foreground Blue
	} else {
		Write-Host "OK (not installed...)" -Foreground Green
	}
}


#-------------------------------------------------------------------------------------------------------------------------
# Ask for elevated permissions for the rest of the script
#-------------------------------------------------------------------------------------------------------------------------
...
# Removes unwanted Apps that come with Windows.
#-------------------------------------------------------------------------------------------------------------------------
Write-Output "Uninstalling default user apps"
foreach ($app in $apps) {
    Write-Host "- Removing $app : " -NoNewLine
	if (Get-AppxPackage -Name $app) {
		Get-AppxPackage -Name $app | Remove-AppxPackage
...
#-------------------------------------------------------------------------------------------------------------------------
# Local Account Passsword Management
#-------------------------------------------------------------------------------------------------------------------------
# Remove Password Age Limit (Passwords never expire) #
net accounts /maxpwage:unlimited
# Set account never expires for local Admin
set-CimInstance -Query 'Select * from Win32_UserAccount where name LIKE "admin%" and localaccount="true"' -Property @{PasswordExpires=$False}
...
# WINDOWS 11
#Set-ItemProperty -Path "HKCU:\Software\Policies\Microsoft\Windows\Explorer" -Name "DisableSearchBoxSuggestions" -Type DWord -Value 0
...
# Removes Widgets button
Write-Host "Hiding Widgets button..."
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "TaskbarDa" -Type DWord -Value 0
# Show Widgets button
# New-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "TaskbarDa" -Type DWord -Value 1

# Hide Chat button
Write-Host "Hiding Chat button..."
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "TaskbarMn" -Type DWord -Value 0
# Show Chat button
# New-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "TaskbarMn" -Type DWord -Value 1

# Default StartMenu alignment 0=Left
Write-Host "Setting StartMenu alignement..."
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "TaskbarAl" -Type DWord -Value 0
# Default StartMenu alignment 1=Center
# New-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "TaskbarAl" -Type DWord -Value 1

# Hide Cortana button
Write-Host "Hiding Cortana button..."
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "ShowCortanaButton" -Type DWord -Value 0
# Show Cortana button
# Remove-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "ShowCortanaButton"
...
$services = @(
    ...
    # "dmwappushservice"                       # WAP Push Message Routing Service (see known issues) - required for Intune / MDM
    # "lfsvc"                                    # Geolocation Service
    # "MapsBroker"                               # Downloaded Maps Manager
    # "NetTcpPortSharing"                      # Net.Tcp Port Sharing Service
    # "RemoteAccess"                           # Routing and Remote Access
    # "RemoteRegistry"                         # Remote Registry
    # "SharedAccess"                           # Internet Connection Sharing (ICS) - required for VPNs and Hyper-V Switch
...
#-------------------------------------------------------------------------------------------------------------------------
# Uninstall McAfee Applications
#-------------------------------------------------------------------------------------------------------------------------
# Uninstall McAfee WebAdvisor if present ###
if (Test-Path -Path ${env:programfiles}\McAfee\WebAdvisor\Uninstaller.exe -PathType Leaf) {
	write-host "- Removing: McAfee WebAdvisor"
	start-process "${env:programfiles}\McAfee\WebAdvisor\Uninstaller.exe" -arg "/s" -Wait
}
## Uninstall McAfee Security if present ###
if (Test-Path -Path ${env:programfiles}\McAfee\MSC\mcuihost.exe -PathType Leaf) {
	write-host "- Removing: McAfee Security"
	start-process "${env:programfiles}\McAfee\MSC\mcuihost.exe" -arg "/body:misp://MSCJsRes.dll::uninstall.html /id:uninstall" -Wait
}
